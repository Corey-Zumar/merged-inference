...
class SimpleNN(Model):
    ...
    def get_input_tensor(self):
        if not self.has_graph:
            raise Exception("Attempted to get input tensor from model whose graph has not been created")

        return self.input_tensor

    def get_output_tensor(self):
        if not self.has_graph:
            raise Exception("Attempted to get output tensor from model whose graph has not been created")

        return self.output_tensor

    def get_inputs(self, num_inputs):
        inps_list = [np.random.rand(*inputs_shape).flatten() for _ in range(num_inputs)]
        return np.array(inps_list, dtype=self.input_dtype)

def main():
    parser = argparse.ArgumentParser(
            description='Test merged inference vs sequential inference on two small neural networks')
    parser.add_argument("-b", "--batch_size", type=int, default=8, help="The query batch size to evaluate")
    parser.add_argument("-g", "--gpu_num", type=int, default=0, help="The number of the GPU on which to execute the
                                                                      experiment")
    ...
    nn_a = SimpleNN(input_shape=input_shape, hl_sizes=[20, 10], activation=None)
    nn_b = SimpleNN(input_shape=input_shape, hl_sizes=[8,6], activation=None)

    eval_sequential([nn_a, nn_b], batch_size=args.batch_size, gpu_num=args.gpu_num)
