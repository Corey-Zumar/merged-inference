...
def eval_model(model, batch_size, gpu_num=0, sess=None):
    ...
    feed_dict = {model.get_input_tensor(): inputs[idxs]}
    before = datetime.now()
    outputs = sess.run(model.get_output_tensor(), feed_dict=feed_dict)
    after = datetime.now()
    latency = (after - before).total_seconds()
    logger.info("{mod} Latency: {lat}".format(
        mod=model.get_name(), lat=latency))
...
def eval_sequential(models, batch_size, gpu_num=0):
    ...
    for model in models:
        latency = eval_model(
            model=model, batch_size=batch_size, gpu_num=gpu_num)
